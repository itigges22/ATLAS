...
| `ATLAS_LLAMA_MEM` | `9728m` | Memory cap on the inference container. It had none, which is precisely why the kernel's victim selection chose it: unbounded and largest. A limit does not make the model use less — it makes the kernel reclaim its page cache under pressure instead of killing it, and it makes the deployment's total a number that can be checked. |
