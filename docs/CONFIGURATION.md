...
| `ATLAS_FRESH_SLOT_PER_SESSION` | `1` | Set to `0` to disable per-session llama.cpp KV-slot erase. With it enabled (default), the proxy POSTs `/slots/<n>?action=erase` for every slot at the start of each agent loop invocation, giving each turn a clean cache. A slot that answers 503 (busy) is retried; a server that answers 4xx or 501 (no slots endpoint, or `--slots` not enabled) has answered and is not retried. Adds ~1-2s to the first turn but prevents cross-session token-state leakage (e.g. filenames hallucinated from prior sessions). |
