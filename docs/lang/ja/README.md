...
1. **[atlas-proxy](../../ARCHITECTURE.md#3-atlas-proxy-outer-layer)** - システム全体をオーケストレーションする Go ベースのエージェントループ。
  ...
  - b. [構造化出力](../../ARCHITECTURE.md#grammar-enforcement) - vLLM の `response_format: json_object` がすべてのツール呼び出し応答を有効な JSON に制約します
...
5. **[vLLM](../../CONFIGURATION.md#6-vLLM)** - 単一のコンシューマ GPU でのローカル LLM 推論。
  - a. CUDA アクセラレーション - AWQ-Q4 量子化モデル推論 (PagedAttention)
...
- **NVIDIA のみでテスト済み** - ATLAS は推論に vLLM を使用しており、複数のアクセラレータバックエンドをサポートしています。ROCm サポートは V3.1 の優先事項です。
...
- **ガイド付きデコーディング推論のスループット** - 単一の tok/s 値は意味を持ちません。vLLM のスループットは並列度・GPU・量子化形式 (AWQ-Q4 vs FP8) で大きく変動します。本番環境では `nvidia-smi dmon` と vLLM のログで実測値を確認してください。
