...
> ℹ️ **抄訳版です。** ATLAS に固定のデフォルトモデルはありません。`atlas init` でレジストリモデルを選ぶか、任意の互換 GGUF を指定してください。最新のモデル非依存セットアップは英語版 ([SETUP.md](../../SETUP.md)) を参照してください。
...
| **モデルウェイト** | `atlas init` で選択したレジストリモデル、または互換 GGUF |
...
```bash
cd /path/to/your/project
atlas    # atlas-proxy への到達性を確認してから TUI を起動します
```
