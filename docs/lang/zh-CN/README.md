...
1. **[atlas-proxy](../../ARCHITECTURE.md#3-atlas-proxy-outer-layer)** - 基于 Go 的代理循环，负责编排整个系统。
  ...
  - b. [结构化输出](../../ARCHITECTURE.md#grammar-enforcement) - vLLM 的 `response_format: json_object` 将所有工具调用响应约束为有效的 JSON
...
5. **[vLLM](../../CONFIGURATION.md#6-vLLM)** - 在单块消费级 GPU 上进行本地 LLM 推理。
  - a. CUDA 加速 - AWQ-Q4 量化模型推理（PagedAttention）
