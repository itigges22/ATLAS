...
5. **[vLLM](../../CONFIGURATION.md#6-vLLM)** - 단일 소비자용 GPU에서의 로컬 LLM 추론입니다.
  - a. CUDA 가속 - AWQ-Q4 양자화된 모델 추론 (PagedAttention)
...
- **NVIDIA에서만 테스트됨** - ATLAS는 추론에 vLLM를 사용하며, 다양한 가속기 백엔드를 지원합니다. ROCm 지원은 V3.1의 우선 과제입니다.
...
- **가이드 디코딩 추론 처리량** - 단일 tok/s 수치는 의미가 없습니다. vLLM 처리량은 동시성, GPU, 양자화 형식 (AWQ-Q4 vs FP8)에 따라 크게 달라집니다. 프로덕션에서는 `nvidia-smi dmon`과 vLLM 로그로 실제 값을 확인하세요.
...
**V3.1** - 진행 중.
- ROCm 지원 - vLLM ROCm 백엔드를 통한 AMD GPU 추론
