...
- **NVIDIA에서만 테스트됨** - ATLAS는 추론에 llama.cpp를 사용하며, 다양한 가속기 백엔드를 지원합니다. ROCm 지원은 다음 릴리스 로드맵에 포함되어 있습니다.
- **9B 모델 공식 벤치마크 미실시** - CLI는 전체 V3 파이프라인과 함께 Qwen3.5-9B를 제공하지만, 공식 LiveCodeBench 점수는 14B 모델 기준입니다. 9B 벤치마크는 다음 릴리스 작업 항목입니다.
- **복잡한 기능 추가 시 실패 가능** - 기존 프로젝트에 기능을 추가하는 작업은 약 67%의 성공률을 보입니다. 모델이 코드를 작성하는 대신 탐색에 과도하게 시간을 소비하는 경우가 있습니다.
- **문법 제약 추론 속도** - llama-server에서 약 51 tok/s입니다. 더 빠른 문법 통합이 다음 릴리스 로드맵에 포함되어 있습니다.

---

## 로드맵

**V3.1.0** - 현재 릴리스. Bubbletea TUI가 공식 채팅 클라이언트 (PC-062), `atlas init` 초기 설정 마법사 (PC-054), `atlas doctor` 진단 도구 (PC-053), `atlas tier` 하드웨어 인식 프리셋 (PC-055), K3s 배포 템플릿 복원, 설치 시 자동 빌드되는 ASA 스티어링 벡터 (BiasBusters #4).
...
- 구조적 코드 추론 - tree-sitter + 솔버 기반 콜그래프를 통한 도달성 쿼리 및 범위 지정 컨텍스트 주입 ([issue #39](https://github.com/itigges22/ATLAS/issues/39))
