...
2. 파일에 **2개 이상의 로직 지표**가 있을 것 (함수 정의, 제어 흐름, API 패턴) — 또는 인식되는 코드/마크업 확장자(`.py`, `.go`, `.js`, `.html` 등)를 가질 것 (이 경우 지표가 없어도 10줄 이상이면 T2)
3. V3 서비스가 `ATLAS_V3_URL`에서 접근 가능할 것

구성, 데이터, 스타일, 마크다운, 셸 파일(`package.json`, `.yaml`, `.css`, `.md`, `.sh` 등)은 크기와 관계없이 항상 T1입니다.
...
**자동으로 수행되는 동작:**
- 기존 파일의 경우: 프록시가 모든 `write_file`을 거부하고 모델에게 `edit_file`을 대신 사용하도록 안내합니다 (예외는 5줄 이하의 파일, 디스크에서 손상된 것으로 보이는 파일, 이 세션이 직접 작성한 파일뿐입니다)
...
**원인:** 모델이 아무것도 쓰지 않고 4회 이상 연속으로 읽기 전용 호출(read_file, search_files, list_directory)을 했습니다. 4회 읽기 시점에 프록시가 쓰기를 유도하는 넛지를 주입하고, 5회 이상이면 더 강한 넛지를 주입합니다. 읽기는 항상 실행됩니다 — 넛지는 다음 턴을 유도할 뿐, 읽기를 건너뛰지 않습니다.
...
- 파일을 10줄 미만으로 유지 (T1 유지, V3 미실행) — 인식되는 코드 확장자는 10줄 이상이면 복잡도와 관계없이 T2가 됩니다
