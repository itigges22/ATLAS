...
**증상:** vLLM 컨테이너가 시작되지만 모델이 CPU에서 로드됩니다 (매우 느림, 약 2 tok/s). 호스트에서 `nvidia-smi`로 GPU가 보이지만 컨테이너에서는 접근할 수 없습니다.
...
**증상:** vLLM 컴파일 중 `docker compose build`가 CUDA 관련 오류로 실패합니다.

**해결:** vLLM Dockerfile은 `nvidia/cuda:12.8.0-devel` 기본 이미지 내에서 vLLM를 빌드하므로, 호스트 GPU 접근 없이도 빌드 시 CUDA 헤더를 사용할 수 있습니다. 빌드 실패의 일반적인 원인:
1. 디스크 공간 부족 (빌드 아티팩트에 약 5GB 필요)
2. CUDA 기본 이미지 다운로드 또는 vLLM 클론 시 네트워크 문제
...
**증상:** vLLM가 "failed to load model" 또는 유사한 메시지와 함께 즉시 종료됩니다.

**해결:** 모델 디렉토리를 확인하십시오 (vLLM은 단일 GGUF 파일이 아니라 AWQ-Q4 safetensors 샤드 디렉토리를 직접 사용합니다):
```bash
...
경로는 `.env`의 `ATLAS_MODEL_PATH`와 일치해야 합니다 (기본값: `/models/Qwen3.5-9B-AWQ`). HuggingFace에서 가져오려면 `make model`을 실행하십시오.

### VRAM 부족

**증상:** vLLM가 시작 직후 충돌하거나 OOMKilled됩니다. `nvidia-smi`에서 VRAM 사용량이 거의 100%로 표시됩니다.

**해결:** 9B AWQ-Q4 빌드는 gen에서 약 12 GB, embed 사이드카에서 약 3 GB의 VRAM이 필요합니다 (둘 다 같은 GPU를 공유). 다음을 확인하십시오:
...
**해결:** 프록시는 `ATLAS_AGENT_LOOP=1`일 때 자동으로 `response_format: {"type": "json_object"}`를 설정합니다. vLLM를 직접 사용하는 경우 요청에 포함하십시오:
```bash
...
JSON 대신 일반 텍스트가 반환되면 vLLM 빌드가 `response_format`을 지원하지 않는 것입니다. 최신 소스에서 다시 빌드하십시오.
...
**증상:** 요청이 vLLM로 직접 전달됩니다. 도구 호출, 스트리밍 상태 아이콘, V3 파이프라인이 없습니다.
...
일반적인 원인:
- Lens가 vLLM에 연결할 수 없음 (`LLAMA_URL` 환경 변수 확인)
...
**원인:** Lens가 vLLM의 `/v1/embeddings` 엔드포인트를 호출합니다. vLLM에 과부하가 걸리거나 해당 엔드포인트가 활성화되지 않으면 실패합니다.
...
`/v1/embeddings` 엔드포인트는 생성 모델의 셀프 임베딩을 위해 특별한 플래그 없이 vLLM에서 사용 가능합니다. K3s에서는 전체 임베딩 지원을 위해 엔트리포인트에 `--embeddings` 플래그가 명시적으로 설정됩니다.
...
**예상 RAM 사용량:**
- vLLM: 약 8 GB (모델은 VRAM에, 최소 RAM 사용)
