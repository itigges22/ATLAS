...
| Limit | Value | Purpose |
|-------|-------|---------|
| Conversation trim | Token-budget driven: keep system + most-recent-user-instruction + as many trailing messages as fit `min(65% of per-slot context, 14000 tokens)` (floor: keep 8; ceiling override via `ATLAS_AGENT_HISTORY_BUDGET`). Pinning the most-recent user (not the first) is critical — long agent loops push the original task beyond a "keep first user" window and the model goes generic. The 14k ceiling matters on sliding-window-attention models: llama.cpp can't reuse the KV cache for them, so the whole history is re-encoded every turn | Prevent context overflow; bound per-turn re-encode cost |
| Redundant-read short-circuit | A whole-file `read_file` of a file already read this session, with on-disk content unchanged, returns a compact "already in context, make your edit" pointer instead of the full file (`ATLAS_DEDUP_READS=0` disables). Paged reads (offset/limit) and changed files always serve the real content | Keeps re-read loops from re-injecting + re-encoding the same file every turn |
