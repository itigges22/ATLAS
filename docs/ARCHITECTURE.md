...
| `lens.go` | Lens scoring calls, lens-sample banking (`/feedback` for human verdicts, plus mechanical labelling: gate rejections as full-weight negatives, verified-run writes as weight-0.5 positives — selected by `verifiedFinalWrites`: only the final write of a path whose on-disk sha256 a green verification actually covered, per the `VerificationRecord` evidence the loop keeps for every passing verification command; a file the passing command never named, or bytes rewritten after the pass, earn nothing), calibration status |
...
**Phase 0: Probe** generates a single baseline candidate with progressive budget retry (light → standard → nothink). It is scored with the selected model's C(x)/G(x) artifacts and tested in the sandbox. If it passes, the pipeline exits immediately. If the self-test oracle scores uniformly 0/N while the probe itself executes cleanly (the answer key is broken, not the code — measured: 42/42 verifications in one run), the pipeline returns unverified immediately (`oracle_inconclusive`) rather than generating candidates no oracle can rank; with `ATLAS_V3_DEAD_ORACLE_CONSENSUS=1` a bounded input-only consensus attempt runs first — at most two extra candidates and four generated inputs, every step gated on remaining wall-clock, unverified return the moment no cluster forms. Off by default pending an A/B.
...
**V3 capability modes.** `v3_mode` on `/v1/agent` selects which V3 capabilities a request may use: `full` (production default — planning and candidate generation), `off` (neither; the wire-compatible spelling of `bypass_v3: true`, which also relaxes the demo-baseline structural gates), and `planner_only` (experimental — the planner and plan-adherence steering run, writes execute through the ordinary guarded path, and candidate generation, PlanSearch, DivSampling, candidate sandbox evaluation, repair, Best-of-K selection, Lens candidate selection and every delivery attempt stay off). Planning and candidate generation are separate capabilities: a single bypass boolean disabled both at once, so an A/B whose arms differ only in `bypass_v3` cannot say which one moved the result. Every candidate-pipeline entry point sits behind `V3GenerationEnabled()`, and a structural test fails on any new site that reads the raw boolean, so `planner_only` cannot reach generation by a forgotten negation. An unrecognised `v3_mode` is a 400, never a silent fall back to `full`.
