...
| [`cli/commands/model_registry.py`](../atlas/cli/commands/model_registry.py) | Honest model registry (PC-056). `Model` dataclass with `lens_status` field (`supported` / `no-artifacts` / `unverified`), `REGISTRY` list, `for_tier()` / `tier_for_model()` / `by_name()` / `is_installed()` lookups. Today only `Qwen3.5-9B-Q6_K` is `supported`; 7B/14B/32B listed honestly as `no-artifacts` (no trained Lens tensors). Bringing more models to `supported` is the work of PC-058. |
...
| [`cli/commands/model.py`](../atlas/cli/commands/model.py) | `atlas model` (PC-056): `list` / `recommend` / `install` / `remove`. Streams downloads via stdlib urllib with progress bar. Lens-status safety gate refuses no-artifacts installs without `--no-lens`. Composes with `tier.classify()` for `recommend`. |
...
| [`test_model_registry.py`](../tests/cli/test_model_registry.py) | 22 cases for PC-056 registry: shape (4 entries, 1 supported, 1 installable), forward + reverse lookups, `is_installed` filesystem probe with sparse-file fixtures, PC-055.2 back-compat shim still resolves identically. |
| [`test_model.py`](../tests/cli/test_model.py) | 21 cases for PC-056 CLI: list filters, recommend composition with mocked tier probe, install safety gates (unknown name, no-artifacts refusal, gated upstream, existing-file overwrite), remove confirm + idempotence, path resolution (env > flag). |
