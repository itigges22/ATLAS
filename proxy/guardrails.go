...
//
// Nothing in this file recognised a request to REMOVE a named file. The write
// verbs cover creation, and actionIntentWords covers changing something that
// exists, so "Delete obsolete.py" registered as no action intent at all while
// the vague "Clean up the old implementation" registered as one. A completion
// rule that wants to know whether a deletion was actually asked for has had
// nothing to ask.
//
// This answers that one question and nothing else. It is not permission to
// delete, it authorises no tool call, and it decides no terminal: it names the
// paths a later slice may treat as obligations, and says "unsupported" for
// everything it cannot read safely.
//
// Precision over recall, deliberately. The failure mode on the other side is
// treating a destructive act as the finished task, so the grammar is small and
// anything outside it fails the WHOLE extraction closed rather than returning
// the part it understood.

// maxDeleteIntentPaths bounds one request. A message naming more paths than
// this is not read as a tidy list of deletions; it is read as unsupported.
const maxDeleteIntentPaths = 8

type deleteIntentStatus int

const (
	// deleteIntentNone: the message does not ask for a deletion at all.
	deleteIntentNone deleteIntentStatus = iota
	// deleteIntentExplicit: every deletion clause was read, and Paths holds
	// the complete canonical set.
	deleteIntentExplicit
	// deleteIntentUnsupported: something about the request is a deletion but
	// cannot be read safely -- ambiguous, optional, a content edit, a glob,
	// an unresolvable path, or more paths than the bound. Never partial.
	deleteIntentUnsupported
)

// deleteIntent is the whole result. Paths is meaningful only when Status is
// deleteIntentExplicit, and is canonical, deduplicated and sorted.
type deleteIntent struct {
	Status deleteIntentStatus
	Paths  []string
	Reason string // why it was unsupported; for logs and tests, never the model
}

// deleteVerbs are the only openings this grammar accepts. "rm " is included
// with its space so it cannot match inside a word.
var deleteVerbs = []string{"delete", "deletes", "deleting", "deleted",
	"remove", "removes", "removing", "removed", "rm "}

// deleteClauseUnsupported are shapes that mean the target is something INSIDE
// a file, or that the clause is not naming one file. Read from the clause the
// verb governs.
var deleteClauseUnsupported = []struct{ token, why string }{
	{" from ", "names something inside a file, not the file"},
	{" in ", "names something inside a file, not the file"},
	{" of ", "names something inside a file, not the file"},
	{" or ", "an alternative target is ambiguous"},
	{"whichever", "the target is not named"},
	{"whatever", "the target is not named"},
	{"any file", "the target is not named"},
	{"every ", "the target is not named"},
	{"all the ", "the target is not named"},
	{"*", "globs are not expanded"},
	{"?", "globs are not expanded"},
	{" to ", "a relocation is not a deletion"},
	{"rename", "a relocation is not a deletion"},
	{"move ", "a relocation is not a deletion"},
}

// deleteHedgeWords make a deletion optional or hypothetical rather than
// requested. They usually sit BEFORE the verb ("you may delete", "feel free to
// remove"), so they are read from a window either side of it.
var deleteHedgeWords = []struct{ token, why string }{
	{"may ", "the deletion is optional"},
	{"can ", "the deletion is optional"},
	{"could ", "the deletion is optional"},
	{"should ", "the deletion is optional"},
	{"might ", "the deletion is optional"},
	{"feel free", "the deletion is optional"},
	{"if needed", "the deletion is optional"},
	{"if necessary", "the deletion is optional"},
	{"if you want", "the deletion is optional"},
	{"if you like", "the deletion is optional"},
	{"optionally", "the deletion is optional"},
	{"consider ", "the deletion is optional"},
	{"whether", "the message is asking, not instructing"},
}

// deleteHedgeWindow is how far before a verb a hedge still governs it.
const deleteHedgeWindow = 40

// deleteClauseNegated are the ways a clause cancels its own deletion from
// INSIDE. Negation from before the verb is negatedAt's job and is reused.
var deleteClauseNegated = []string{"neither", " nor "}

// explicitDeleteIntent reads a user message for explicit requests to make named
// paths absent.
//
// Clause-scoped: each delete verb owns the text up to the next clause
// terminator, so "Delete obsolete.py; do not delete config.py" authorises
// obsolete.py and not config.py, in either order. A negated clause is skipped;
// an unreadable one fails everything.
func explicitDeleteIntent(ctx *AgentContext, msg string) deleteIntent {
	lower := strings.ToLower(msg)
	// A question about deleting is not a request to delete.
	if strings.Contains(msg, "?") {
		if hasDeleteVerb(lower) {
			return deleteIntent{Status: deleteIntentUnsupported,
				Reason: "the message is asking, not instructing"}
		}
		return deleteIntent{Status: deleteIntentNone}
	}
	if isExplainOnlyMessage(lower) || isReadOnlyRequest(msg) {
		if hasDeleteVerb(lower) {
			return deleteIntent{Status: deleteIntentUnsupported,
				Reason: "the message asks about the workspace rather than changing it"}
		}
		return deleteIntent{Status: deleteIntentNone}
	}
	...
	seen := map[string]bool{}
	var paths []string
	sawVerb, sawAuthorising := false, false

	for _, at := range deleteVerbOffsets(lower) {
		sawVerb = true
		clause := deleteClauseAt(lower, at)
		// Cancelled from before the verb ("do not", "instead of", ...) or from
		// inside it ("neither ... nor"). Either way this clause authorises
		// nothing, and the rest of the message still stands on its own.
		if negatedAt(lower, at) || containsAny(clause, deleteClauseNegated) {
			continue
		}
		for _, u := range deleteClauseUnsupported {
			if containsWord(clause, u.token) {
				return deleteIntent{Status: deleteIntentUnsupported, Reason: u.why}
			}
		}
		// A hedge reaches backwards: "you may delete cache.py" is optional,
		// and the word that makes it optional is not in the clause.
		start := at - deleteHedgeWindow
		if start < 0 {
			start = 0
		}
		around := lower[start:at] + clause
		for _, h := range deleteHedgeWords {
			if containsWord(around, h.token) {
				return deleteIntent{Status: deleteIntentUnsupported, Reason: h.why}
			}
		}
		found := deleteClausePaths(msg, lower, at, len(clause))
		if len(found) == 0 {
			return deleteIntent{Status: deleteIntentUnsupported,
				Reason: "a deletion was asked for without a usable path"}
		}
		sawAuthorising = true
		for _, p := range found {
			...
			if err != nil {
				return deleteIntent{Status: deleteIntentUnsupported,
					Reason: "a named path is not inside the workspace"}
			}
			if seen[canon] {
				continue // the same file spelled two ways is one obligation
			}
			if len(paths) >= maxDeleteIntentPaths {
				return deleteIntent{Status: deleteIntentUnsupported,
					Reason: "more paths than this can account for"}
			}
			seen[canon] = true
			paths = append(paths, canon)
		}
	}
	if !sawVerb || !sawAuthorising {
		return deleteIntent{Status: deleteIntentNone}
	}
	sort.Strings(paths)
	return deleteIntent{Status: deleteIntentExplicit, Paths: paths}
}

func hasDeleteVerb(lower string) bool { return len(deleteVerbOffsets(lower)) > 0 }

// deleteVerbOffsets returns every position a delete verb starts at, in order.
func deleteVerbOffsets(lower string) []int {
	var out []int
	for _, v := range deleteVerbs {
		for idx := 0; ; {
			i := strings.Index(lower[idx:], v)
			if i < 0 {
				break
			}
			at := idx + i
			// Word start only: "removed" must not also match inside another
			// token, and "undelete" is not "delete".
...
// deleteClauseAt returns the clause a verb governs: from the verb to the next
// terminator. Conjunctions inside a list are not terminators, so "a.py, b.py,
// and c.py" stays one clause.
func deleteClauseAt(lower string, at int) string {
	rest := lower[at:]
	end := len(rest)
	for _, term := range []string{";", ".\n", "\n", ", but ", " but ", " then ", " however "} {
		if i := strings.Index(rest, term); i >= 0 && i < end {
			end = i
		}
	}
	// A sentence-ending period is a terminator; a period inside a filename is
	// not, so only one followed by a space or end-of-string counts.
	for i := 0; i < end; i++ {
...
// deleteClausePaths pulls filename tokens out of one clause, using the same
// tokenizer the output-path parser uses.
func deleteClausePaths(msg, lower string, at, clauseLen int) []string {
	end := at + clauseLen
	if end > len(msg) {
		end = len(msg)
	}
	var out []string
	for _, m := range reOutputFilenameTok.FindAllStringSubmatchIndex(msg[at:end], -1) {
		p := msg[at:end][m[2]:m[3]]
		if strings.TrimSpace(p) == "" || strings.Count(p, ".") == len(p) {
			continue
		}
		out = append(out, p)
	}
...
// containsWord is Contains with a left word boundary, so "move " does not
// match inside "remove " and "can " does not match inside "scan ". Tokens that
// already begin with a space or punctuation are unaffected.
func containsWord(s, token string) bool {
	if token == "" || !isWordByte(token[0]) {
		// Already anchored by its own leading space or punctuation.
