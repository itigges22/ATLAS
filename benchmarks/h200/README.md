...
- `launch_on_h200.sh` — on the H200: build llama.cpp container, start server with `--parallel 16`, kick off benchmarks
...
- Confirmation the instance has:
  ...
  - CUDA 12.9 available on the host

If the provider ships a pre-built PyTorch image, that's fine — we'll build llama.cpp ourselves inside a container.
...
~7GB of model + maybe 100MB of code and state. 10-20 min depending on upload bandwidth.
...
This builds the container (~5 min), starts llama-server on port 32735 with `--parallel 16`, smoke-tests it, and then runs baseline + ATLAS in sequence.
