...
$CRT run -d --name atlas-bench \
    ...
    -e EMBED_MAX_NUM_BATCHED_TOKENS="${EMBED_MAX_NUM_BATCHED_TOKENS:-}" \
    -e LLAMA_GEN_MODEL="${LLAMA_GEN_MODEL:-qwen3.5-9b}" \
    -e LLAMA_EMBED_MODEL="${LLAMA_EMBED_MODEL:-qwen3.5-9b-embed}" \
    -e BENCHMARK_PARALLEL="${BENCHMARK_PARALLEL:-16}" \
    -e ATLAS_LLM_PARALLEL="${ATLAS_LLM_PARALLEL:-1}" \
    -e ATLAS_PARALLEL_TASKS="${ATLAS_PARALLEL_TASKS:-16}" \
    "$IMAGE_TAG"
